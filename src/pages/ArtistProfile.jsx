...
import ArtistProfilePreviewCardModal from '../components/artist/ArtistProfilePreviewCardModal';
import ArtistArtGalleryPopup from '../components/artist/ArtistArtGalleryPopup';
import ArtistProfileActiveArtView from '../components/artist/ArtistProfileActiveArtView';
...
import './ArtistProfile.bottomModal.css';
import './ArtistProfile.bottomAuthGallery.css';
import './ArtistProfile.bottomPublicArt.css';
import './ArtistProfile.modals.css';
import './ArtistProfile.artPopup.css';
...
const ArtistProfile = ({ artistData }) => {
    ...
    const MOCK_ARTIST = {
        ...
        name: 'Elena Vasquez',
        specialization: 'Micro-Sculptor & Installation Artist',
        bio: '10 years crafting worlds smaller than a thumbnail. My work explores the intersection of nature and technology.',
        photo: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=400&h=400&fit=crop',
        backgroundPhoto: 'https://images.unsplash.com/photo-1513364776144-60967b0f800f?w=1200&h=400&fit=crop',
        email: 'mock@artist.com',
        ...
        // Default theme + default font for the mock preview
        profileTheme: 'mono',
        profileFont: 'outfit',
        bioFont: 'outfit'
    };

    const [artist, setArtist] = useState(isMock ? MOCK_ARTIST : (artistData || null));
    const [loading, setLoading] = useState(artistData ? false : !isMock);
    ...
    useEffect(() => {
        if (isMock) {
            // Avoid Firebase auth calls in mock mode.
            setUser(null);
    ...
    useEffect(() => {
        if (isMock) {
            ...
            setFormData({
                ...
                specialization: MOCK_ARTIST.specialization || '',
                bio: MOCK_ARTIST.bio || '',
                photo: MOCK_ARTIST.photo || '',
                backgroundPhoto: MOCK_ARTIST.backgroundPhoto || '',
                gallery: MOCK_ARTIST.gallery || [],
                email: MOCK_ARTIST.email || '',
                phone: MOCK_ARTIST.phone || '',
                website: MOCK_ARTIST.website || '',
                instagram: MOCK_ARTIST.instagram || '',
                facebook: MOCK_ARTIST.facebook || '',
                twitter: MOCK_ARTIST.twitter || '',
                whatsapp: MOCK_ARTIST.whatsapp || '',
                linkedin: MOCK_ARTIST.linkedin || '',
                instagramName: MOCK_ARTIST.instagramName || '',
                instagramCategory: MOCK_ARTIST.instagramCategory || '',
                instagramPosts: MOCK_ARTIST.instagramPosts || '',
                instagramFollowers: MOCK_ARTIST.instagramFollowers || '',
                instagramFollowing: MOCK_ARTIST.instagramFollowing || '',
                instagramAccountBio: MOCK_ARTIST.instagramAccountBio || '',
                profileTheme: MOCK_ARTIST.profileTheme || 'mono',
                profileFont: MOCK_ARTIST.profileFont || 'outfit',
                bioFont: MOCK_ARTIST.bioFont || (MOCK_ARTIST.profileFont || 'outfit')
            });
            return;
        }
    ...
    }, [token, artistIdParam, artistData, isMock]);
    ...
    // REGULAR PROFILE VIEW â€“ profile only, no flip
    ...
    if (activeArt) {
        return (
            <ArtistProfileActiveArtView
                artist={artist}
                activeArt={activeArt}
                activeArtImages={activeArtImages}
                showPhotoModal={showPhotoModal}
                setShowPhotoModal={setShowPhotoModal}
                selectedImage={selectedImage}
                setSelectedImage={setSelectedImage}
                artLangOpen={artLangOpen}
                artLangSearch={artLangSearch}
                setArtLangSearch={setArtLangSearch}
                artLangLoading={artLangLoading}
                setArtLangLoading={setArtLangLoading}
                artLangError={artLangError}
                setArtLangError={setArtLangError}
                setArtLangOpen={setArtLangOpen}
                artSelectedLang={artSelectedLang}
                setArtSelectedLang={setArtSelectedLang}
                artDescriptionTranslated={artDescriptionTranslated}
                setArtDescriptionTranslated={setArtDescriptionTranslated}
            />
        );
    }
        ...
        <div className={`artist-profile-wrapper theme-${activeTheme}`}>
            ...
            >
                ...
                <div className="profile-info-content">
                        ...
                        <div className="artist-swiper-wrap fade-in" onClick={(e) => e.stopPropagation()}>
                            {slideGallery.length > 0 ? (
                                ...
                                <div className="mini-slideshow-placeholder">
                                    <div className="placeholder-icon">ðŸ–¼ï¸</div>
                    ...
                    {/* Art Gallery Popup */}
                    <ArtistArtGalleryPopup
                        show={showArtGallery}
                        artist={artist}
                        artGallerySelected={artGallerySelected}
                        onCloseGallery={() => { setShowArtGallery(false); setArtGallerySelected(null); }}
                        onSelectArtItem={(item) => setArtGallerySelected(item)}
                        onCloseLightbox={() => setArtGallerySelected(null)}
                    />
            ...
            <ArtistProfilePreviewCardModal
                show={showPhotoModal}
                onClose={() => { setShowPhotoModal(false); setSelectedImage(null); }}
                selectedImage={selectedImage}
                artist={artist}
            />
        </div>
